...
class MAEViTEncoder(nn.Module):
    ...
    def __init__(self, patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
        ...
        self.blocks = nn.ModuleList([
            Block(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer)
            for i in range(depth)])
